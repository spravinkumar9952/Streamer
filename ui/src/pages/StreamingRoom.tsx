...
interface SocketEvents {
  play: (roomId: string, email: string | undefined) => void;
  pause: (roomId: string, email: string | undefined) => void;
...
export const StreamingRoomPlayer: FC<StreamingRoomProps> = ({ streamingRoomObj }) => {
  ...
  const handlePause = useCallback(() => {
    if (!roomId) return;
    emitEvent("pause", roomId, user?.email);
  }, [roomId, user?.email, emitEvent]);

  const handleSeek = useCallback((seconds: number) => {
    console.log("handleSeek called with seconds:", seconds);
    if (!roomId || !user?.email) {
      console.log("Cannot emit seek event - missing roomId or user email", { roomId, userEmail: user?.email });
      return;
    }
    console.log("Emitting seek event with:", { roomId, userEmail: user?.email, seconds });
  ...
  const handleProgress = useCallback((state: OnProgressProps) => {
    ...
    if (!roomId || !user?.email) {
      console.log("Cannot emit progress event - missing roomId or user email", { roomId, userEmail: user?.email });
      return;
    }
    console.log("Emitting progress event with:", { roomId, userEmail: user?.email, playedSeconds: state.playedSeconds });
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-background-card rounded-card shadow-card p-6 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <h2 className="text-2xl font-bold text-text-primary">{streamingRoomObj.name}</h2>
            ...
            <div className="flex items-center space-x-3">
              <div className="flex -space-x-2">
                <img src="/png/users.png" alt="Viewer" className="w-8 h-8 rounded-full border-2 border-background-card" />
                <img src="/png/users.png" alt="Viewer" className="w-8 h-8 rounded-full border-2 border-background-card" />
          ...
          <div className="bg-background-card rounded-card shadow-card overflow-hidden">
            <div className="relative w-full" style={{ paddingTop: '56.25%' }}> {/* 16:9 Aspect Ratio */}
              <div className="absolute top-0 left-0 w-full h-full">
                <ReactPlayer
                  ref={playerRef}
                  url={streamingRoomObj.videoUrl || "https://youtu.be/wo_e0EvEZn8?feature=shared"}
          ...
          <div className="bg-background-card rounded-card shadow-card p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center space-x-2 px-3 py-1 bg-background-secondary rounded-full">
                ...
                <span className="text-sm font-medium text-text-secondary">Room active for 2h 30m</span>
              </div>
            </div>
